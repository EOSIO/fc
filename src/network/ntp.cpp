...
namespace fc
{
    ...
    public:
        ...
        for( auto item : _ntp_hosts )
        {
          try
          {
            ...
            for( auto ep : eps )
            {
              ...
              send_buf_as_64_array[5] = fc_time_point_to_ntp_timestamp(fc::clock::now()); // 5 = Transmit Timestamp
      ...
      void read_loop()
      {
        ...
        while( true )
        {
          ...
          fc::time_point receive_time = fc::clock::now();
          ...
          else //we think we have a timely reply, process it
          {
            if( offset < fc::seconds(60*60*24) && offset > fc::seconds(-60*60*24) )
            {
              ...
              elog( "NTP time and local time vary by more than a day! ntp:${ntp_time} local:${local}",
                   ("ntp_time", receive_time + offset)("local", fc::clock::now()) );
          }
        }
      ...
      void reschedule() {
          ...
          fc::clock::now() + fc::seconds(_request_interval_sec) );
      }
    }; //ntp_impl
  ...
  optional<time_point> ntp::get_time()const
  {
    if( my->_last_ntp_delta_initialized )
      return fc::clock::now() + fc::microseconds(my->_last_ntp_delta_microseconds);
